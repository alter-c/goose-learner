...
ENC_FEAT_SIZE = len(LTG_FEATURES)
VAR_FEAT_SIZE = 4

LTG_EDGE_LABELS = OrderedDict(
    {
        "neutral": 0,
        "ground": 1,
        "pre_pos": 2,
...
DEGREE_FEAT_SIZE = len(LTG_EDGE_LABELS)

class LiftedTopologyGraph(Representation, ABC):
    name = "ltg"
    n_node_features = ENC_FEAT_SIZE + VAR_FEAT_SIZE + DEGREE_FEAT_SIZE
    n_edge_labels = len(LTG_EDGE_LABELS)
    directed = False
    lifted = True
    ...
    def _feature(self, node_type: LTG_FEATURES) -> Tensor:
    ...
    def _compute_graph_representation(self) -> None:
        ...
        for i, obj in enumerate(self.problem.objects):
            G.add_node(obj.name, x=self._feature(LTG_FEATURES.O))  # add object node

        # predicates
        largest_predicate = 0
        for pred in self.problem.predicates:
            largest_predicate = max(largest_predicate, len(pred.arguments))
            G.add_node(pred.name, x=self._feature(LTG_FEATURES.P))  # add predicate node

        # fully connected between objects and predicates
        for pred in self.problem.predicates:
            for obj in self.problem.objects:
                self.add_edge_with_node_feature(
                    G, u_of_edge=pred.name, v_of_edge=obj.name, edge_label=LTG_EDGE_LABELS["neutral"]
                )
        ...
        for fact in goals:
            ...
            if is_negated:
                x = self._feature(LTG_FEATURES.N)
                self._neg_goal_nodes.add(goal_node)
            else:
                x = self._feature(LTG_FEATURES.G)
            ...
            for i, arg in enumerate(args):
                ...
                self.add_edge_with_node_feature(
                    ...
                    edge_label=LTG_EDGE_LABELS["ground"],
                )
            ...
            self.add_edge_with_node_feature(G, u_of_edge=goal_node, v_of_edge=pred, edge_label=LTG_EDGE_LABELS["ground"])
        # end goal

        # actions
        largest_action_schema = 0
        for action in self.problem.actions:
            G.add_node(action.name, x=self._feature(LTG_FEATURES.A))
            action_args = {}
            ...
            for i, arg in enumerate(action.parameters):
                ...
                self.add_edge_with_node_feature(
                    G, u_of_edge=action.name, v_of_edge=arg_node, edge_label=LTG_EDGE_LABELS["neutral"]
                )
            ...
            def deal_with_action_prec_or_eff(predicates, edge_label):
                for z, predicate in enumerate(predicates):
                    ...
                    assert pred in G.nodes()
                    self.add_edge_with_node_feature(
                        G, u_of_edge=pred, v_of_edge=aux_node, edge_label=LTG_EDGE_LABELS[edge_label]
                    )
                    ...
                    if len(predicate.args) > 0:
                        for j, arg in enumerate(predicate.args):
                            ...
                            self.add_edge_with_node_feature(
                                ...
                                edge_label=LTG_EDGE_LABELS[edge_label],
                            )
    ...
    def state_to_tensor(self, state: List[Tuple[str, List[str]]]) -> TGraph:
        ...
        for fact in state:
            ...
            if node in self._node_to_i:
                x[self._node_to_i[node]][LTG_FEATURES.S.value] = 1
                continue

            # activated proposition does not overlap with a goal
            true_node_i = i
            x[i][LTG_FEATURES.S.value] = 1
            i += 1
            ...
            x[true_node_i][-DEGREE_FEAT_SIZE + edge_label] += 1
            x[self._node_to_i[pred]][-DEGREE_FEAT_SIZE + edge_label] += 1
            ...
            for k, arg in enumerate(args):
                ...
                x[true_node_i][-DEGREE_FEAT_SIZE + edge_label] += 1
                x[true_var_node_i][-DEGREE_FEAT_SIZE + edge_label] += 1
                ...
                x[true_var_node_i][-DEGREE_FEAT_SIZE + edge_label] += 1
                x[self._node_to_i[arg]][-DEGREE_FEAT_SIZE + edge_label] += 1

        # 对每个节点的x特征的最后几个维度的拓扑特征进行归一化
        degree_features = x[:, -DEGREE_FEAT_SIZE:]
        degree_norms = torch.norm(degree_features, dim=1, keepdim=True)
        # 避免除零
        epsilon = 1e-8
        degree_norms = torch.clamp(degree_norms, min=epsilon)
        # 归一化拓扑特征
        x[:, -DEGREE_FEAT_SIZE:] = degree_features / degree_norms
        

        edge_indices[LTG_EDGE_LABELS["ground"]] = torch.hstack(
            (edge_indices[LTG_EDGE_LABELS["ground"]], torch.tensor(append_edge_index).T)
        ).long()
    ...
    def state_to_cgraph(self, state: List[Tuple[str, List[str]]]) -> CGraph:
        ...
        for fact in state:
            ...
            # connect fact to predicate
            c_graph.add_edge(u_of_edge=node, v_of_edge=pred, edge_label=LTG_EDGE_LABELS["ground"])
            c_graph.add_edge(v_of_edge=node, u_of_edge=pred, edge_label=LTG_EDGE_LABELS["ground"])
            ...
            for k, arg in enumerate(args):
                ...
                # connect variable to predicate
                c_graph.add_edge(
                    u_of_edge=node, v_of_edge=arg_node, edge_label=LTG_EDGE_LABELS["ground"]
                )
                c_graph.add_edge(
                    v_of_edge=node, u_of_edge=arg_node, edge_label=LTG_EDGE_LABELS["ground"]
                )

                # connect variable to object
                c_graph.add_edge(
                    u_of_edge=arg_node, v_of_edge=arg, edge_label=LTG_EDGE_LABELS["ground"]
                )
                c_graph.add_edge(
                    v_of_edge=arg_node, u_of_edge=arg, edge_label=LTG_EDGE_LABELS["ground"]
                )

        return c_graph
    
    def add_edge_with_node_feature(self, graph, u_of_edge, v_of_edge, edge_label):
        """Add an edge to the graph with a specific feature."""
        graph.add_edge(u_of_edge=u_of_edge, v_of_edge=v_of_edge, edge_label=edge_label)

        graph.nodes[u_of_edge]["x"][-DEGREE_FEAT_SIZE + edge_label] += 1
        graph.nodes[v_of_edge]["x"][-DEGREE_FEAT_SIZE + edge_label] += 1
