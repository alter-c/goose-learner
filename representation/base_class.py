...
class Representation(ABC):
    ...
    def convert_to_pyg(self) -> None:
        ...
        self.x = pyg_G.x # 节点特征数据(N * F)

        if self.n_edge_labels == 1:
            self.edge_index = pyg_G.edge_index # 边索引数据(2 * E) E为边总数(无向图,边索引为2倍边数量), 2表示连接的两点
        else:
            assert self.n_edge_labels > 1
            self.edge_indices = [[] for _ in range(self.n_edge_labels)] # 边索引数据(2 * E_l) E_l为标签数
