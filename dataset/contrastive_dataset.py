...
def get_graphs_from_plan(domain_pddl, problem_pddl, plan_file, args):
    ...
    # 添加目标状态
    goal_x, goal_index = rep.goal_to_tensor()
    goal_graph = Data(x=goal_x, edge_index=goal_index, y=0)
    graphs.append(goal_graph)

    # 正样本索引：下一个graph的索引
    positive_index = [i+1 for i in range(len(graphs))]
    positive_index[-1] = len(graphs) - 2  # 目标状态的正样本是最后一个状态

    return graphs, positive_index
...
def get_graphs_from_dir(domain_pddl, tasks_dir, plans_dir, args):
    ...
    for plan_file in sorted(list(os.listdir(plans_dir))):
        ...
        graphs, positive_index = get_graphs_from_plan(domain_pddl, problem_pddl, plan_file, args)
        all_graphs.extend(graphs)
        all_positive_index.extend([p + len(all_graphs) - len(graphs) for p in positive_index])
    return all_graphs, all_positive_index
...
def get_graphs_from_args(args):
    ...
    if domain == "ipc":
        ...
        graphs, positive_index = get_graphs_from_dir(domain_pddl, tasks_dir, plans_dir, args)
...
def get_loaders_from_args_gnn(args):
    ...
    graphs, positive_index = get_graphs_from_args(args)
    # 数据构成: 图数据列表 + 正样本索引列表
    if small_train:
        random.seed(721)
        indices = random.sample(range(len(graphs)), k=1000)
        graphs = [graphs[i] for i in indices]
        positive_index = [positive_index[i] for i in indices]

    indices = list(range(len(graphs)))
    train_index, val_index = train_test_split(indices, test_size=0.15, random_state=4550)

    trainset = ContrastiveDataset(graphs, positive_index, train_index)
    valset = ContrastiveDataset(graphs, positive_index, val_index)

    get_stats(dataset=graphs, desc="Whole dataset")
    get_stats(dataset=trainset[0], desc="Train set")
    get_stats(dataset=valset[0], desc="Val set")
...
class ContrastiveDataset(Dataset):
    def __init__(self, graphs, positive_index, split_index):
        self.all_graphs = graphs
        self.graphs = [graphs[i] for i in split_index]
        self.positive_index = [positive_index[i] for i in split_index]
    ...
    def __getitem__(self, idx):
        ...
        pos_idx = self.positive_index[idx]
        positive = self.all_graphs[pos_idx]
