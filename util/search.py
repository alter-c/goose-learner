...
_EXPERIMENT_DIR = "../experiments"
_AUX_DIR = f"{_EXPERIMENT_DIR}/aux_dir"
_PLAN_DIR = f"{_EXPERIMENT_DIR}/plans"
...
def fd_cmd(df, pf, m, search, timeout=TIMEOUT):
    """
    生成FastDownward搜索命令
    :param df: Domain PDDL file
    :param pf: Problem PDDL file
    :param m: 模型文件路径
    :param search: 搜索策略 (例如"gbfs")
    :param timeout: 搜索时限

    :return: 命令字符串, SAS规划文件路径
    """
    if search == "gbbfs":
