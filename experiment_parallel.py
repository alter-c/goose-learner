...
from util.search import FAIL_LIMIT, fd_cmd, sorted_nicely
...
_MODEL_NAME = "topology_degree"
_EXPERIMENT_DIR = f"../experiments/{_MODEL_NAME}"

_TRAINED_MODEL_DIR = f"{_EXPERIMENT_DIR}/{_MODEL_NAME}_all_trained_models"
_VALIDATED_MODEL_DIR = f"{_EXPERIMENT_DIR}/{_MODEL_NAME}_trained_models"

_MAIN_LOG_DIR = f"{_EXPERIMENT_DIR}/{_MODEL_NAME}_logs"
...
def get_model_desc(rep, domain, L, H, aggr, patience, val_repeat):
    return f"{_MODEL_NAME}_{rep}_{domain}_L{L}_H{H}_{aggr}_p{patience}_v{val_repeat}"


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-r", "--representation", default="ltg", choices=REPRESENTATIONS)
    ...
    def run_experiment_for_domain(args_tuple):
        ...
        desc = f"{_MODEL_NAME}_{rep}_{domain}_L{L}_H{H}_{aggr}_p{patience}"
        ...
        # test on problems
        # TODO 添加并行测试
        def test_single_problem(f, desc, domain, rep, test_dir, df, model_file, test_log_dir, timeout, fail_limit):
            test_log_file = f"{test_log_dir}/{f.replace('.pddl', '')}_{desc}.log"
            finished_correctly = False
        ...
        test_files = sorted_nicely(os.listdir(test_dir))
        num_threads = min(2, len(test_files))  # 你可以根据需要调整线程数
        fail_limit = FAIL_LIMIT[domain]
        success = 0
        failed = 0
        ...
        func = partial(
            ...
            test_log_dir=_LOG_DIR_TEST,
            timeout=TIMEOUT,
            fail_limit=fail_limit
        )
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            future_to_file = {executor.submit(func, f): f for f in test_files}
        ...
        print(f"已完成测试: 测试任务为 {domain}, 图表示为 {rep}.")
        print(f"规划任务总数: {len(os.listdir(test_dir))}.")
        print(f"规划成功任务数: {success}.")
        print(f"规划覆盖率: {success / len(os.listdir(test_dir))}.")

    # Prepare all combinations
    domain_args = list(product(Ls, aggrs, DOMAINS))

    # Use multiprocessing to run experiments in parallel
    with multiprocessing.Pool(processes=min(len(DOMAINS), os.cpu_count())) as pool:
        pool.map(run_experiment_for_domain, domain_args)
        
    # for L, aggr, domain in product(Ls, aggrs, DOMAINS):
    #     val_dir = f"../dataset/goose/{domain}/val"
    #     test_dir = f"../dataset/goose/{domain}/test"
